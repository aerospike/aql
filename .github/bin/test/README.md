...
runner, against the signed package after it is installed. It checks that aql runs and that
**the version it reports matches the repo's `VERSION` file**, so a package labelled with one
version but carrying a binary stamped with another fails here instead of downstream in the
aerospike-tools bundle.

Both printed lines are compared. `print_version()` splits the embedded string on `-`, so
`9.2.10-rc1` reports `Version 9.2.10` / `Build rc1` and a bare `9.2.10` reports no `Build`
