...
Build Docker images for Aerospike aql (Ubuntu 24.04).
...
REQUIRED:
    -v, --version VERSION   aql version, e.g. 3.0.0 or 3.0.0-rc1
...
DISTROS AND BASE IMAGES:
    ubuntu24.04    Ubuntu 24.04, installs aql .deb   (linux/amd64 + linux/arm64)

TAGS PRODUCED:
  Test mode (loaded locally):
    <reg>/aerospike-aql:<version>-<distro>-<arch>
    e.g. aerospike/aerospike-aql:3.0.0-ubuntu24.04-amd64
...
EXAMPLES:
    ...
    docker/docker-build.sh -t -v 3.0.0-rc1 -u /path/to/dist

    # Build + push to JFrog dev registry
    docker/docker-build.sh -p -v 3.0.0 \
    ...
    docker/docker-build.sh -t -v 3.0.0 -S

    # Stitch manifest after native per-arch pushes
    docker/docker-build.sh -M -v 3.0.0 -d ubuntu24.04 \
        -r artifact.aerospike.io/database-docker-dev-local
...
function _emit_test_target() {
  ...
  if [[ -n "${local_pkg}" ]]; then
    echo "  args = { AQL_LOCAL_PKG = \"${local_pkg}\" }"
  fi
  echo "  tags = ["
  for ((i = 0; i < n; i++)); do
    if [[ $i -lt $((n - 1)) ]]; then
...
function _emit_push_target() {
  ...
  if [[ -n "${local_pkg_amd64}" || -n "${local_pkg_arm64}" ]]; then
    echo "  args = {"
    [[ -n "${local_pkg_amd64}" ]] && echo "    AQL_LOCAL_PKG_AMD64 = \"${local_pkg_amd64}\""
    [[ -n "${local_pkg_arm64}" ]] && echo "    AQL_LOCAL_PKG_ARM64 = \"${local_pkg_arm64}\""
    echo "  }"
  fi
  echo "  tags = ["
  for ((i = 0; i < n; i++)); do
...
function _emit_single_arch_push_target() {
  ...
  if [[ -n "${local_pkg}" ]]; then
    if [[ "${arch}" == "amd64" ]]; then
      echo "  args = { AQL_LOCAL_PKG_AMD64 = \"${local_pkg}\" }"
    else
      echo "  args = { AQL_LOCAL_PKG_ARM64 = \"${local_pkg}\" }"
    fi
  fi
  echo "  tags = ["
  for ((i = 0; i < n; i++)); do
...
function run_manifest_mode() {
  ...
  for distro in "${ACTIVE_DISTROS[@]}"; do
    for reg in "${REGISTRY_PREFIXES[@]}"; do
      ...
      src_amd64="${reg}/aerospike-aql:${VERSION}-${distro}-amd64"
      src_arm64="${reg}/aerospike-aql:${VERSION}-${distro}-arm64"
      if [[ "${multi_distro}" == true ]]; then
        target_tag="${reg}/aerospike-aql:${VERSION}-${distro}"
      else
...

  {
    ...
    for distro in "${ACTIVE_DISTROS[@]}"; do
      ...
      for arch in "${ACTIVE_ARCHES[@]}"; do
        ...
        local tags=()
        for reg in "${REGISTRY_PREFIXES[@]}"; do
          tags+=("${reg}/aerospike-aql:${VERSION}-${distro}-${arch}")
        done
    ...
    if [[ "${PUSH_SINGLE_ARCH:-false}" == true ]]; then
      ...
      for distro in "${ACTIVE_DISTROS[@]}"; do
        ...
        for reg in "${REGISTRY_PREFIXES[@]}"; do
          if [[ "${multi_distro}" == true ]]; then
            tags+=("${reg}/aerospike-aql:${VERSION}-${distro}")
            if [[ -n "${TIMESTAMP}" ]]; then
              tags+=("${reg}/aerospike-aql:${VERSION}-${distro}-${TIMESTAMP}")
            fi
          else
  ...
  for distro in "${ACTIVE_DISTROS[@]}"; do
    ...
    ubuntu24.04)
      if [[ "${need_amd64}" == true ]]; then
        LOCAL_PKG_UBUNTU_AMD64=$(_copy_pkg "aerospike-aql_*_ubuntu24.04_x86_64.deb" "ubuntu24.04")
      fi
      if [[ "${need_arm64}" == true ]]; then
        LOCAL_PKG_UBUNTU_ARM64=$(_copy_pkg "aerospike-aql_*_ubuntu24.04_aarch64.deb" "ubuntu24.04")
      fi
      ;;
    esac
  done
