...
# update-version.sh
#
# Updates the version, download URLs, and SHA256 checksums in the aql Dockerfiles.
#
# JFrog URL structure:
#   DEB: {deb-base}/pool/{ubuntu-codename}/aerospike-aql/{filename}
#        e.g. .../pool/noble/aerospike-aql/aerospike-aql_3.0.0-rc1_ubuntu24.04_x86_64.deb
#
# Usage:
#   ./docker/update-version.sh --version VERSION [OPTIONS]
#
# Required:
#   --version VERSION       aql version, e.g. 3.0.0 or 3.0.0-rc1
#
# Source options:
...
#                           Full URL: {deb-base}/pool/{codename}/aerospike-aql/{filename}
#
# Other options:
...
#   ./docker/update-version.sh --version 3.0.0-rc1 --packages-dir /path/to/pkgs
#
#   # Remote URL, no SHA256 (fastest):
#   ./docker/update-version.sh --version 3.0.0-rc1
#
#   # Remote URL + download to compute SHA256:
#   ./docker/update-version.sh --version 3.0.0-rc1 --compute-sha
#
#   # Preview without modifying files:
#   ./docker/update-version.sh --version 3.0.0-rc1 --packages-dir /path/to/pkgs --dry-run
...
# Package filenames
# ---------------------------------------------------------------------------
PKG_UBUNTU2404_AMD64="aerospike-aql_${VERSION}_ubuntu24.04_x86_64.deb"
PKG_UBUNTU2404_ARM64="aerospike-aql_${VERSION}_ubuntu24.04_aarch64.deb"

# ---------------------------------------------------------------------------
# Construct download URLs
# ---------------------------------------------------------------------------
DEB_POOL="${DEB_BASE_URL}/pool/${UBUNTU2404_CODENAME}/aerospike-aql"
...
update_dockerfile() {
    ...
    sed -i.bak \
        -e "s|^ARG AQL_AMD64_URL=.*|ARG AQL_AMD64_URL=\"${amd64_url}\"|" \
        -e "s|^ARG AQL_AMD64_SHA256=.*|ARG AQL_AMD64_SHA256=\"${amd64_sha}\"|" \
        -e "s|^ARG AQL_ARM64_URL=.*|ARG AQL_ARM64_URL=\"${arm64_url}\"|" \
        -e "s|^ARG AQL_ARM64_SHA256=.*|ARG AQL_ARM64_SHA256=\"${arm64_sha}\"|" \
        "${file}"
    rm -f "${file}.bak"
    echo "  Updated: ${file}"
}
