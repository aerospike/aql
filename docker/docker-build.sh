...
TAGS PRODUCED:
  Test mode (loaded locally):
    <reg>/aerospike-aql:<version>-<arch>
    e.g. aerospike/aerospike-aql:5.0.0-amd64

  Push mode — multi-arch (default, both arches):
    <reg>/aerospike-aql:<version>
    <reg>/aerospike-aql:<version>-<timestamp>   (when -T is set)
...
function run_manifest_mode() {
  ...
  for reg in "${REGISTRY_PREFIXES[@]}"; do
    ...
    src_amd64="${reg}/aerospike-aql:${VERSION}-amd64"
    src_arm64="${reg}/aerospike-aql:${VERSION}-arm64"
    target_tag="${reg}/aerospike-aql:${VERSION}"
...
function generate_bake() {
...

  {
    ...
    for arch in "${ACTIVE_ARCHES[@]}"; do
      ...
      local tags=()
      for reg in "${REGISTRY_PREFIXES[@]}"; do
        tags+=("${reg}/aerospike-aql:${VERSION}-${arch}")
      done
    ...
    if [[ "${PUSH_SINGLE_ARCH:-false}" == true ]]; then
      ...
      local tags=()
      for reg in "${REGISTRY_PREFIXES[@]}"; do
        tags+=("${reg}/aerospike-aql:${VERSION}")
        if [[ -n "${TIMESTAMP}" ]]; then
          tags+=("${reg}/aerospike-aql:${VERSION}-${TIMESTAMP}")
        fi
      done
