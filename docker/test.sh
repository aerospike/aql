...
# docker/test.sh
#
# Smoke-test locally-built (or pullable) Aerospike aql Docker images.
#
...
Smoke-test Aerospike aql Docker images.

REQUIRED:
    -v, --version VERSION   Expected aql version string, e.g. 3.0.0 or 3.0.0-rc1
...
EXAMPLES:
    ...
    docker/test.sh -v 3.0.0-rc1

    # Test all arches for all distros
    docker/test.sh -v 3.0.0 -a amd64 -a arm64

    # Test a specific image tag
    docker/test.sh -i aerospike/aerospike-aql:3.0.0-ubuntu24.04-amd64 -v 3.0.0

    # Test + Snyk scan + cleanup
    docker/test.sh -v 3.0.0 -s -c
EOF
}
...
function run_snyk() {
  ...
  local scan_log="aerospike-aql-${version}-${distro:+${distro}-}${arch}-${ts}.snyk"
